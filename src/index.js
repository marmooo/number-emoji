...
loadAudio("correct2", "/number-emoji/mp3/papa1.mp3");
loadAudio("correctAll", "/number-emoji/mp3/levelup1.mp3");
loadConfig();

function loadConfig() {
  if (localStorage.getItem("darkMode") == 1) {
    document.documentElement.setAttribute("data-bs-theme", "dark");
    colorNode.value = "#ffffff";
  }
}
...
function toggleDarkMode() {
  if (localStorage.getItem("darkMode") == 1) {
    ...
    if (colorNode.value == "#ffffff") colorNode.value = "#000000";
  } else {
    localStorage.setItem("darkMode", 1);
    document.documentElement.setAttribute("data-bs-theme", "dark");
    if (colorNode.value == "#000000") colorNode.value = "#ffffff";
  }
}

function changeColor() {
  for (const text of svg.querySelectorAll("text")) {
    text.setAttribute("fill", colorNode.value);
  }
}
...
function changeLang() {
  ...
  location.href = `/number-emoji/${lang}/`;
}
...
function addNumber(x, y, r, i, pathIndex, display) {
  const fillColor = colorNode.value;
...
async function fetchIconList(course) {
  const response = await fetch(`/number-emoji/data/${course}.txt`);
...
colorNode.onchange = changeColor;
